...
{ 
  ...
  std::string str1 = "percent uncorrelated smearing";
  const char* atitle = str1.c_str();

  std::string str2="scintdual4";
  ...
  A_pt->GetYaxis()->SetTitle("Ratio dual-corrected to scint resolution  ");  
  A_pt->GetYaxis()->SetTitleSize(0.05);  
  A_pt->GetXaxis()->SetTitle(atitle);  
  A_pt->GetXaxis()->SetTitleSize(0.05);  
  A_pt->GetXaxis()->SetRangeUser(0.,0.07);
  //A_pt->GetYaxis()->SetRangeUser(0.,1.2);
